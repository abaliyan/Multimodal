...
test_y_lst = [[370, 4.9], [340, 6], [530, 17.1], [240, 28], [1066, 9.1], [842, 8.2], [610, 15.6], [368, 3.6], [365, 13.4], [776, 8.08], [1068, 22.72], [1990, 12.28], [436, 5.74]]
    ...
    for foldName, model_data_list in models_dict.items():
        if foldName == 'OctaDataSet':
            ...
            for index, (xafs, xrd) in enumerate(zip(spectral_mixup_XAFS, spectral_mixup_XRD)):
                ...
                for model in model_data_list:
                    if model['model'].__class__.__name__ == 'LinearRegression':
                        ...
                        for index, (score, aug_data) in enumerate(zip(predictedscore1, augmented_data_list)):
                            # Get MA and SA from your test_y_lst
                            ma, sa = score[index]

                            # Build dict with Index, Predicted, score
                            row = {
                                'Index': index,
                                'Predicted': ma,
                                'score': sa
                ...
                results_df.to_excel(f"{DIR_Data}/Aug_Predicted_Octa_MA_SA.xlsx", index=False)
