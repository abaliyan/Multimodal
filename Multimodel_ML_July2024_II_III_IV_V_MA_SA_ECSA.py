...
train_y_lst = [[490, 10.9, 45], [480, 8.5, 57], [540, 5.7, 95], [271, 6, 45], [360, 4.4, 82], [400, 5.2, 77], [175, 3.6, 49], [370, 4.9, 76], [340, 6, 56], [330, 8, 43], [530, 17.1, 31], [630, 16.3, 39],
               ...
               [324, 3.8, 84], [312, 4.1, 77], [319, 4.6, 70], [368, 3.6, 103], [323, 3.2, 101], [366, 3.5, 105], [385, 12.8, 39], [334, 12, 37], [365, 13.4, 35],
               [841, 8.76, 96], [675, 7.35, 92], [776, 8.08, 96], [704, 7.18, 98], [701, 7.01, 100], [1300, 23.64, 55], [1053, 19.5, 54], 
...
test_y_lst = [[370, 4.9, 76], [340, 6, 56], [530, 17.1, 31], [240, 28, 8], [1066, 9.1, 117], [842, 8.2, 103], [610, 15.6, 39.3], [368, 3.6, 103], [365, 13.4, 35], [776, 8.08, 96], [1068, 22.72, 47], [1990, 12.28, 162], [436, 5.74, 76]]
    ...
    for foldName, model_data_list in models_dict.items():
        if foldName == 'OctaDataSet':
            ...
            for index, (xafs, xrd) in enumerate(zip(spectral_mixup_XAFS, spectral_mixup_XRD)):
                ...
                for model in model_data_list:
                    if model['model'].__class__.__name__ == 'LinearRegression':
                        ...
                        for index, (score, aug_data) in enumerate(zip(predictedscore1, augmented_data_list)):
                            # Unpack MA, SA, ECSA from your test_y_lst
                            ma, sa, ecsa = test_y_lst[index]

                            # Build dict with all target columns
                            row = {
                                'Index': index,
                                'Predicted': ma,
                                'score': sa,
                ...
                results_df.to_excel(f"{DIR_Data}/Aug_Predicted_Octa_MA_SA_ECSA.xlsx", index=False)
