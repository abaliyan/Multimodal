"""Multi-modal Machine Learning Pipeline for Combined MA and SA Prediction.

This script implements a comprehensive ML pipeline for jointly predicting
Mass Activity (MA) and Specific Activity (SA) from multi-modal spectroscopic
data including XANES, EXAFS, XRD, PDF, SAXS, and HAXPES measurements.
"""
...
def ReadCsvData(filename):
    """Read and preprocess spectroscopic data from Excel files for joint MA/SA prediction.

    This function handles different spectroscopic techniques with technique-specific
    data range selections and preprocessing steps optimized for combined Mass Activity
    and Specific Activity prediction.
...
def checkPermutationsnew(models, DataArray, x_lst, y_lst, training_seed=334, num_evaluation_seeds=1):
    """Execute comprehensive permutation-based cross-validation for joint MA/SA prediction.
    ...
    for combined Mass Activity and Specific Activity prediction.
    ...
    Args:
        ...
        y_lst (list): Target values (combined MA and SA measurements)
    ...
    Key Features:
        ...
        - Multi-target optimization for both MA and SA properties
    """
    ...
    for pair in range(len(modalities), 0, -1):  # Start from 8 for OctaDataSet
        ...
        output_filename = f"{DIR_Data}/models_scores_MA_SA/"

        for model_idx, original_model in enumerate(models):
            model_filename = f"{output_filename}{foldName}_model_{model_idx+1}_{original_model.__class__.__name__}_MA_SA.pkl"
    ...
    for pair in range(len(modalities), 0, -1):  # Start from 8 for OctaDataSet
        ...
        for cv_seed in num_evaluation_seeds:
            ...
            output_filename = f"{DIR_Data}/models_scores_MA_SA/"
            results_file = f"{output_filename}{cv_foldName}_MA_SA_model_mse_scores.xlsx"
    ...
    if all_cv_results:
        ...
        for pair in range(len(modalities), 0, -1):
            ...
            output_filename = f"{DIR_Data}/models_scores_MA_SA/"
            phase1_excel_file = f"{output_filename}{phase1_foldName}_MA_SA_model_mse_scores.xlsx"
...
def Training_Testing_models(listofmodels, X, Y, groups, X_test, Y_test, groups_test, foldName, log_file=None):
    """Train and evaluate ML models for combined MA and SA prediction.

    This function implements the core training and testing pipeline for joint MA/SA prediction,
    training multiple ML algorithms on spectroscopic features and evaluating their
    performance on held-out test data for both electrochemical properties.
    ...
    Args:
        ...
        Y (array-like): Training target values (combined MA and SA measurements)
    ...
    Key Metrics Calculated:
        - R² score on training and test sets for both MA and SA
        - Mean Absolute Error (MAE) for both targets
        - Feature importance rankings
        - Model-specific parameters and metadata
        - Multi-target regression performance assessment
    """
    X_train = np.array(X)
    Y_train = np.array(Y)
    ...
    for index, original_model in enumerate(listofmodels):
        ...
        # Save model to file
        model_filename = f"{output_filename}{foldName}_model_{index+1}_{model.__class__.__name__}_MA_SA.pkl"
    ...
    df_results.to_excel(
        f"{output_filename}{foldName}_MA_SA_model_mse_scores.xlsx", index=False)
...
train_y_lst =  [[490, 10.9], [480, 8.5], [540, 5.7], [271, 6], [360, 4.4], [400, 5.2], [175, 3.6], [370, 4.9], [340, 6], [330, 8], [530, 17.1], [630, 16.3],
                [390, 8.1], [380, 7.8], [240, 28], [315, 32], [490, 5.5], [811, 7], [1066, 9.1], [842, 9.7], [670, 7.2], [790, 8.1], [842, 8.2], [490, 7.2], [610, 15.6],
                [324, 3.8], [312, 4.1], [319, 4.6], [368, 3.6], [323, 3.2], [366, 3.5], [385, 12.8], [334, 12], [365, 13.4],
                [841, 8.76], [675, 7.35], [776, 8.08], [704, 7.18], [701, 7.01], [1300, 23.64], [1053, 19.5],
                [1068, 22.72], [772, 19.79], [1990, 12.28], [675, 7.67], [436, 5.74]]
...
Ridge_model = r"models_scores_MA_SA/OctaDataSet_Seed334_model_1_Ridge_MA_SA.pkl"
XGB_model = r"models_scores_MA_SA/OctaDataSet_Seed334_model_4_XGBRegressor_MA_SA.pkl"
...
for i in models_list:
    ...
    importances_mean.to_excel(f"{DIR_Data}/{str(type(loaded_model).__name__)}_Feature_importance_MA_SA.xlsx")
