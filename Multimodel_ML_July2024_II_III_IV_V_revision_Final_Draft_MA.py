"""Multi-modal Machine Learning Pipeline for Mass Activity (MA) Prediction.

This script implements a comprehensive ML pipeline specifically for predicting
Mass Activity (MA) from multi-modal spectroscopic data including XANES, EXAFS,
XRD, PDF, SAXS, and HAXPES measurements.
"""
...
def ReadCsvData(filename):
    ...
    data range selections and preprocessing steps optimized for MA prediction.
...
def PlotCustumFigure(data, foldername='', Data_type='', XaxisFlag=True, xdata=np.empty(0), RangeStart=0, RangeEnd=0, NormFlag=True):
    """Generate customized plots for individual spectroscopic datasets.

    Creates publication-ready plots with optional normalization and range selection
    for visualization of spectroscopic data used in MA prediction.

    Args:
        data (array-like): Spectroscopic intensity data
        foldername (str): Output directory for saved plots
        Data_type (str): Spectroscopic technique identifier for labeling
        XaxisFlag (bool): Whether to use custom x-axis data
        xdata (array): Custom x-axis values (energy, q-vector, etc.)
        RangeStart (int): Starting index for data trimming
        RangeEnd (int): Ending index for data trimming
        NormFlag (bool): Whether to apply min-max normalization
    """
    if len(data) != 0 and np.isnan(data).any() == False:
        fig = plt.figure(1, figsize=(5, 4))
        # Apply data range selection for focused analysis
...
def PlotCustumFigureChar(datadict, dictkey):
    """Generate overlay plots for characterization data comparison.

    Creates overlaid plots showing multiple samples for the same spectroscopic
    technique to enable visual comparison and pattern recognition for MA analysis.

    Args:
        datadict (dict): Nested dictionary containing spectroscopic data
        dictkey (str): Key identifying the spectroscopic technique
    """
...
def PlotCustumFigureMulti(data, foldername='', Data_type='', XaxisFlag=True, RangeStart=0, RangeEnd=0, NormFlag=True):
    """Generate plots for multiple datasets with consistent formatting.

    Creates comparative visualization plots showing multiple samples or conditions
    overlaid for pattern recognition and quality assessment in MA prediction.

    Args:
        data (list): List of spectroscopic datasets to overlay
        foldername (str): Output directory for saved plots
        Data_type (str): Spectroscopic technique identifier
        XaxisFlag (bool): Whether to use indexed x-axis
        RangeStart (int): Starting index for data trimming
        RangeEnd (int): Ending index for data trimming
        NormFlag (bool): Whether to apply min-max normalization
    """
    if len(data) != 0:
        fig = plt.figure(1, figsize=(5, 4))
        # Apply consistent data range selection across multiple datasets
        data = data[RangeStart:RangeEnd]
...
def checkPermutationsnew(models, DataArray, x_lst, y_lst, training_seed=334, num_evaluation_seeds=1):
    """Execute comprehensive permutation-based cross-validation for MA prediction.

    This function implements a two-phase approach for robust model evaluation:
    Phase 1: Train models on all spectroscopic data combinations using a single seed
    Phase 2: Evaluate the best models using multiple random seeds for statistical robustness

    The function systematically tests all possible combinations of spectroscopic techniques
    (from single modality to octa-modal combinations) to identify the optimal feature set
    for Mass Activity prediction.

    Args:
        models (list): List of sklearn-compatible ML models to evaluate
        DataArray (dict): Dictionary containing spectroscopic data for each technique
        x_lst (list): Training feature vectors (combined spectroscopic data)
        y_lst (list): Target Mass Activity values
        training_seed (int): Random seed for initial model training phase
        num_evaluation_seeds (int): Number of seeds for statistical evaluation

    Returns:
        dict: Best performing models with their metadata and performance metrics

    Key Features:
        - Systematic evaluation of 1-8 modality combinations (255 total combinations)
    ...
    for pair in range(len(modalities), 0, -1):  # Start from 8 for OctaDataSet
        ...
        for model_idx, original_model in enumerate(models):
            model_filename = f"{output_filename}{foldName}_model_{model_idx+1}_{original_model.__class__.__name__}_MA.pkl"
    ...
    for pair in range(len(modalities), 0, -1):  # Start from 8 for OctaDataSet
        ...
        for cv_seed in num_evaluation_seeds:
            ...
            output_filename = f"{DIR_Data}/models_scores_MA/"
            results_file = f"{output_filename}{cv_foldName}_MA_model_mse_scores.xlsx"
    ...
    if all_cv_results:
        ...
        for pair in range(len(modalities), 0, -1):
            ...
            output_filename = f"{DIR_Data}/models_scores_MA/"
            phase1_excel_file = f"{output_filename}{phase1_foldName}_MA_model_mse_scores.xlsx"
...
def Training_Testing_models(listofmodels, X, Y, groups, X_test, Y_test, groups_test, foldName, log_file=None):
    """Train and evaluate ML models for Mass Activity prediction.

    This function implements the core training and testing pipeline for MA prediction,
    ...
    Args:
        ...
        Y (array-like): Training target values (Mass Activity measurements)
    ...
    df_results.to_excel(
        f"{output_filename}{foldName}_MA_model_mse_scores.xlsx", index=False)
...
def gp_posterior_mean(K, y, sigma):
    ...
    Computes the posterior mean of a Gaussian Process given kernel matrix,
    observations, and noise level. Used for generating synthetic spectroscopic
    data to augment the MA prediction training dataset.

    Args:
        K (array): Kernel (covariance) matrix between training points
        y (array): Observed target values (Mass Activity measurements)
        sigma (float): Noise standard deviation parameter

    Returns:
        array: Posterior mean predictions for augmented data generation
    """
    identity_matrix = np.identity(K.shape[0])
    return np.matmul(np.matmul(K, np.linalg.inv(K + sigma**2 * identity_matrix)), y)

# Advanced data augmentation using spectral mixing techniques
...
def gp_posterior_cov(K, sigma):
    ...
    Computes the posterior covariance matrix for GP-based data augmentation,
    providing uncertainty estimates for synthetic spectroscopic data generation.
    ...
    Returns:
        array: Posterior covariance matrix for uncertainty quantification
    """
    identity_matrix = np.identity(K.shape[0])
...
def Data_FI(DataArray, DataArray_New, x_lst, y_lst, customsequence=''):
    """Perform Feature Importance analysis with data augmentation for MA prediction.

    This function implements comprehensive feature importance analysis by:
    1. Training models on different combinations of spectroscopic techniques
    2. Applying GP-based and spectral mixup data augmentation
    3. Calculating permutation-based feature importance scores
    4. Generating detailed performance and importance reports

    Args:
        DataArray (dict): Original spectroscopic data dictionary
        DataArray_New (dict): Augmented spectroscopic data dictionary
        x_lst (list): Combined feature vectors for training
        y_lst (list): Target Mass Activity values
        customsequence (tuple, optional): Specific combination of techniques to analyze

    Returns:
        Various outputs including trained models, performance metrics, and
        feature importance rankings for MA prediction optimization.

    Key Processes:
        - Multi-modal data combination and preprocessing
        - Gaussian Process and spectral mixup augmentation
        - Permutation-based feature importance calculation
        - Cross-validation with multiple random seeds
        - Comprehensive performance logging and visualization
    """
    dataList = []
    for key in DataArray:
        dataList.append(key)
...
# Initialize ML models for Mass Activity prediction
# Selected models optimized for spectroscopic feature analysis
...
# Creating paths for all spectroscopic data files
PathDict = {}
...
# Configure data normalization (disabled for raw spectroscopic analysis)
normalizeFlag = False
...
Ridge_model = r"models_scores_MA/OctaDataSet_Seed334_model_1_Ridge_MA.pkl"
XGB_model = r"models_scores_MA/OctaDataSet_Seed334_model_4_XGBRegressor_MA.pkl"
...
for i in models_list:
    ...
    importances_mean.to_excel(f"{DIR_Data}/{str(type(loaded_model).__name__)}_Feature_importance_MA.xlsx")
