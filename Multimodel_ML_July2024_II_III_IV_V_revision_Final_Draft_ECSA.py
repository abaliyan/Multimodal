"""Multi-modal Machine Learning Pipeline for ECSA Prediction.

This script implements a comprehensive ML pipeline specifically for predicting
Electrochemical Surface Area (ECSA) from multi-modal spectroscopic data including
XANES, EXAFS, XRD, PDF, SAXS, and HAXPES measurements.
"""
...
def ReadCsvData(filename):
    """Read and preprocess spectroscopic data from Excel files.

    Handles different spectroscopic techniques with specific data range requirements:
    - XRD: skip first and last 50 points
    - XANES: remove last 86 points
    - EXAFS: limit to first 281 points
    - PDF: skip first 50 points
    - HAXPES_VB: skip first 77 points
    - HAXPES_Pt4f, HAXPES_Pt3d, SAXS: use all data
    """
    try:
        Localfiledata = []
        if "XRD" in filename:
...
def PlotCustumFigure(data, foldername='', Data_type='', XaxisFlag=True, xdata=np.empty(0), RangeStart=0, RangeEnd=0, NormFlag=True):
    """Create and save custom plots for spectroscopic data.

    Args:
        data: Spectroscopic intensity data
        foldername: Directory name for saving plots
        Data_type: Type of spectroscopic technique
        XaxisFlag: Whether to use custom x-axis data
        xdata: X-axis values (energy, angle, etc.)
        RangeStart, RangeEnd: Data range limits
        NormFlag: Whether to normalize data before plotting
    """
...
def PlotCustumFigureChar(datadict, dictkey):
    """Plot multiple spectroscopic datasets for comparison."""
...
def PlotCustumFigureMulti(data, foldername='', Data_type='', XaxisFlag=True, RangeStart=0, RangeEnd=0, NormFlag=True):
    """Create multi-panel plots for spectroscopic data analysis."""
    if len(data) != 0:
        fig = plt.figure(1, figsize=(5, 4))
        # Truncate data to specified range
        data = data[RangeStart:RangeEnd]
...
def save_figure_to_folder(figure, folder_name='', file_name=''):
    """Save matplotlib figures to organized directory structure."""
    if folder_name != '':
...
def save_figure_to_folderMulti(figure, folder_name='', file_name=''):
    """Save multi-panel figures to directory structure."""
    if folder_name != '':
        localresultpath = f"{DIR_Data}" + '/' + folder_name
    if folder_name == 'SingleDataSet':
        localresultpath = f"{DIR_Data}/Analysis/SingleDataSet/"
...
def max_index(lst):
    """Find maximum value and its index in a list."""
    max_val = max(lst)
    max_index = lst.index(max_val)
    return max_val, max_index

def min_index(lst):
    """Find minimum value and its index in a list."""
    min_val = min(lst)
    min_index = lst.index(min_val)
    return min_val, min_index

def getDefaultData(DatatypeName, DataDictionary, DataNameLitral, plotIndividualFlag, datatype):
    """Extract and organize spectroscopic data for ML processing.

    Returns minimum and maximum data lengths for interpolation/extrapolation.
    """
    counter = 0
    ...
    # Find min/max array lengths for interpolation/extrapolation
...
def interpolatedata(old_x, old_y, new_x, type='linear'):
    """Interpolate spectroscopic data to new x-axis grid."""
...
def extrapolatedata(old_x, old_y, new_x, type=1):
    """Extrapolate spectroscopic data to extended x-axis range."""
...
def fixedlendata(old_x, old_y, new_x, type=1):
    """Resample data to fixed length grid."""
    f = interp1d(old_x, old_y, type, fill_value='extrapolate')
...
def getInterXtrapolatedata(Data_old, flag, NormFlag, minElement, maxElement, key_max, key_min):
    """Apply interpolation, extrapolation, or fixed-length resampling to datasets.

    Args:
        flag: 'Inter' for interpolation, 'Xtra' for extrapolation, 'fixed' for fixed-length
        NormFlag: Whether to normalize resampled data
    """
    Data_New = {}
    Data_New_XY = {}
    if flag == 'Xtra':
...
def listofMLmodels(models):
    """Create list of ML model objects for training."""
    MLmodellist = []
    for model in models:
...
def customConcatinate(datalist):
    """Concatenate multiple spectroscopic datasets horizontally."""
    result = datalist[0]
...
def prepareXandYdata(listofdata_x, DataDictionary, listofdata_y):
    """Prepare feature (X) and target (Y) datasets for ML training."""
...
def plotTrainResult(train, test, gtest, legendtxt, foldername='', Data_type=''):
    """Plot training and testing results for model evaluation.

    Handles both single-target and multi-target (ECSA, SA, MA) predictions.
    """
...
def checkPermutationsnew(models, DataArray, x_lst, y_lst, training_seed=334, num_evaluation_seeds=1):
    """Comprehensive cross-validation training pipeline for ECSA prediction.

    This function implements a two-phase training approach:
    Phase 1: Train models on primary seed and save to disk
    Phase 2: Cross-validation training on multiple seeds to prevent overfitting

    Args:
        models: List of ML models to train
        DataArray: Dictionary of spectroscopic data arrays
        x_lst: List of sample identifiers for features
        y_lst: List of ECSA target values
        training_seed: Primary random seed for model training
        num_evaluation_seeds: Number of seeds for cross-validation

    Returns:
        Dictionary containing model paths, CV results, and summary statistics
    """
    ...
    for pair in range(len(modalities), 0, -1):  # Start from 8 for OctaDataSet
        ...
        output_filename = f"{DIR_Data}/models_scores_ECSA/"

        for model_idx, original_model in enumerate(models):
            model_filename = f"{output_filename}{foldName}_model_{model_idx+1}_{original_model.__class__.__name__}_ECSA.pkl"
    ...
    for pair in range(len(modalities), 0, -1):  # Start from 8 for OctaDataSet
        ...
        for cv_seed in num_evaluation_seeds:
            ...
            output_filename = f"{DIR_Data}/models_scores_ECSA/"
            results_file = f"{output_filename}{cv_foldName}_ECSA_model_mse_scores.xlsx"
    ...
    if all_cv_results:
        ...
        for pair in range(len(modalities), 0, -1):
            ...
            output_filename = f"{DIR_Data}/models_scores_ECSA/"
            phase1_excel_file = f"{output_filename}{phase1_foldName}_ECSA_model_mse_scores.xlsx"
...
def Training_Testing_models(listofmodels, X, Y, groups, X_test, Y_test, groups_test, foldName, log_file=None):
    """Train and evaluate ML models for ECSA prediction.

    Args:
        listofmodels: List of ML models to train
        X, Y: Training features and targets
        groups: Training sample groups
        X_test, Y_test: Testing features and targets
        groups_test: Testing sample groups
        foldName: Name for saving models and results
        log_file: Optional log file for training progress

    Returns:
        Empty dictionary (models saved to disk for memory efficiency)
    """
    X_train = np.array(X)
    Y_train = np.array(Y)
    ...
    for index, original_model in enumerate(listofmodels):
        ...
        # Save model to file
        model_filename = f"{output_filename}{foldName}_model_{index+1}_{model.__class__.__name__}_ECSA.pkl"
    ...
    df_results.to_excel(
        f"{output_filename}{foldName}_ECSA_model_mse_scores.xlsx", index=False)
...
def resampleDataset(DataDictionary, DataNameLitral, DataArray, chartechnique, NormFlag):
    """Resample spectroscopic datasets to equivalent spacing grid."""
...
def resampleDatasetfixed(dictkey, DataArray_New):
    """Resample datasets to fixed length (300 points) with normalization."""
    Data_NewF = {}
    Data_New_XYF = {}
    desiredlength = 300
    NormFlag = True
...
def get_number_from_prefix(filename):
    """Extract number from dataset prefix (e.g., 'Double' -> 2)."""
    prefix_map = {
        "Double": 2,
        "Triple": 3,
        "Quad": 4,
...
def data_get(models, DataArray, DataArray_New, x_lst, y_lst, test_x_lst, test_y_lst, customsequence='', sampledetails={}):
    """Create permutations of spectroscopic datasets for ML training.

    Concatenates multiple spectroscopic techniques according to custom sequence.
    """
    dataList = []
...
def gp_posterior_mean(K, y, sigma):
    """Calculate Gaussian Process posterior mean for data augmentation."""
...
def gp_posterior_cov(K, sigma):
    """Calculate Gaussian Process posterior covariance matrix."""
    identity_matrix = np.identity(K.shape[0])
...
def Data_FI(DataArray, DataArray_New, x_lst, y_lst, customsequence=''):
    """Prepare feature data for Feature Importance analysis.

    Concatenates spectroscopic data according to custom sequence for permutation importance.
    """
    dataList = []
    for key in DataArray:
        dataList.append(key)
...
# Configure dataset paths and spectroscopic technique parameters
# Dataset configuration
folder_path = 'MM_dataset_II_III_IV_V_Aug2024/'
filetype = '.xlsx'

# Spectroscopic techniques array
CharArray = ['XANES', 'EXAFS_K2', 'XRD_2_P', 'PDF',
             'SAXS', 'HAXPES_VB', 'HAXPES_Pt4f', 'HAXPES_Pt3d']

# Mapping of techniques to array indices
...
# Machine learning models for ECSA prediction
...
# Generate file paths for all spectroscopic data files
PathDict = {}
...
# Load spectroscopic data from all file paths
DataDictionary = {}
...
# Optional: Apply normalization to raw spectroscopic data
# Currently disabled to preserve original signal characteristics
normalizeFlag = False
...
# Optional: Generate plots for all raw spectroscopic measurements
# Currently disabled for computational efficiency
plotDataFlag = False
...
# Data resampling method selection
# Options: 'Inter' (interpolation), 'Xtra' (extrapolation), 'fixed' (fixed length)
InterXtrapolateFlag = 'Inter'

# Apply interpolation/extrapolation to create consistent data grids
NormFlag = False  # Preserve original signal scaling
DataArray_New = {}
DataArray_New_XY = {}
...
# Apply fixed-length resampling (300 points) for consistent feature dimensions
...
# Define training dataset sample identifiers and ECSA target values
...
# ECSA target values (m²/g-Pt) for each training sample
train_y_lst = [45, 57, 95, 45, 82, 77, 49, 76, 56, 43, 31, 39,
               48.4, 48.7, 8, 10, 89, 116, 117, 87, 93, 97, 103, 68, 39.3,
               84, 77, 70, 103, 101, 105, 39, 37, 35,
               96, 92, 96, 98, 100, 55, 54,
               47, 39, 162, 88, 76]

def get_filename(index):
    """Map combination size index to dataset name."""
    filenames = ['SingleDataSet', 'DoubleDataSet', 'TripleDataSet', 'QuadDataSet',
                 'PentaDataSet', 'HexaDataSet', 'HeptaDataSet', 'OctaDataSet']
    return filenames[index]


# Define optimal spectroscopic technique sequence for ECSA prediction
customsequence = ('EXAFS_K2', 'XRD_2_P', 'XANES', 'PDF',
                  'HAXPES_VB', 'SAXS', 'HAXPES_Pt3d', 'HAXPES_Pt4f')

# Execute comprehensive ML training pipeline (set to False to skip)
customflag = False
if customflag == True:
    try:
        print("Starting comprehensive ECSA prediction training...")
        models_dict = checkPermutationsnew(
            MLmodellist, DataArray_New, train_x_lst, train_y_lst)
        print("ECSA training pipeline completed successfully!")
    except Exception as e:
        print(f"Error during ECSA training: {e}")

# Pre-trained model paths for feature importance analysis
Ridge_model = r"models_scores_ECSA/OctaDataSet_Seed334_model_1_Ridge_ECSA.pkl"
XGB_model = r"models_scores_ECSA/OctaDataSet_Seed334_model_4_XGBRegressor_ECSA.pkl"

models_list = [ f"{DIR_Data}/{Ridge_model}", f"{DIR_Data}/{XGB_model}",]

# Feature importance analysis using pre-trained models
for i in models_list:
    ...
    # Calculate permutation importance for overall feature ranking
    ...
    importances_mean.to_excel(f"{DIR_Data}/{str(type(loaded_model).__name__)}_Feature_importance_ECSA.xlsx")

    results_data = []
    
    prefix = get_number_from_prefix(i)
    # Generate all possible permutations for detailed feature analysis
    ...
    for selected_data in permutations:
        ...
        # Calculate feature contribution by spectroscopic technique (300 features per technique)
...
# ============================================================================
# DATA AUGMENTATION PIPELINE USING GAUSSIAN PROCESSES
# ============================================================================

# Extract specific spectroscopic regions for augmentation
spectra_XAFS = X_train[:,0:300]    # EXAFS features (first 300)
spectra_XRD = X_train[:,300:600]   # XRD features (next 300)

# Generate synthetic EXAFS spectra using Gaussian Process regression
desiredspectrum = 1000//len(spectra_XAFS)  # Number of synthetic spectra per original
...
# Generate synthetic XRD spectra using Gaussian Process with different kernel parameters
# Use different gamma for XRD to capture distinct spectral characteristics
x = np.linspace(0, spectra_XAFS.shape[1], num=spectra_XAFS.shape[1]).reshape(-1, 1)
sigma = 0.01 #0.08
...
# Create augmented dataset by combining synthetic and original spectral features
counter = 0
...
with open(log_file, 'a') as file:
    ...
    # Combine synthetic EXAFS, XRD with original remaining features
    augmented_data_list = []
    ...
    # Test augmented data on trained ECSA models and log predictions
