"""Multi-modal Machine Learning Pipeline for Electrochemical Property Prediction.

This script implements a comprehensive ML pipeline for predicting electrochemical properties
(Mass Activity - MA, Specific Activity - SA, Electrochemical Surface Area - ECSA) from
multi-modal spectroscopic data including XANES, EXAFS, XRD, PDF, SAXS, and HAXPES.
"""
...
def checkPermutationsnew(models, DataArray, x_lst, y_lst, training_seed=334, num_evaluation_seeds=1):
    ...
    for pair in range(len(modalities), 0, -1):  # Start from 8 for OctaDataSet
        ...
        output_filename = f"{DIR_Data}/models_scores_MA_SA_ECSA/"

        for model_idx, original_model in enumerate(models):
            model_filename = f"{output_filename}{foldName}_model_{model_idx+1}_{original_model.__class__.__name__}_MA_SA_ECSA.pkl"
    ...
    for pair in range(len(modalities), 0, -1):  # Start from 8 for OctaDataSet
        ...
        for cv_seed in num_evaluation_seeds:
            ...
            output_filename = f"{DIR_Data}/models_scores_MA_SA_ECSA/"
            results_file = f"{output_filename}{cv_foldName}_MA_SA_ECSA_model_mse_scores.xlsx"
    ...
    if all_cv_results:
        ...
        for pair in range(len(modalities), 0, -1):
            ...
            output_filename = f"{DIR_Data}/models_scores_MA_SA_ECSA/"
            phase1_excel_file = f"{output_filename}{phase1_foldName}_MA_SA_ECSA_model_mse_scores.xlsx"
...
def Training_Testing_models(listofmodels, X, Y, groups, X_test, Y_test, groups_test, foldName, log_file=None):
    ...
    for index, original_model in enumerate(listofmodels):
        ...
        # Save model to file
        model_filename = f"{output_filename}{foldName}_model_{index+1}_{model.__class__.__name__}_MA_SA_ECSA.pkl"
    ...
    df_results.to_excel(
        f"{output_filename}{foldName}_MA_SA_ECSA_model_mse_scores.xlsx", index=False)
...
train_y_lst = [[490, 10.9, 45], [480, 8.5, 57], [271, 6, 45], [360, 4.4, 82], [400, 5.2, 77], [175, 3.6, 49], [370, 4.9, 76], [340, 6, 56], [330, 8, 43], [530, 17.1, 31], 
               [390, 8.1, 48.4], [380, 7.8, 48.7], [240, 28, 8], [315, 32, 10], [490, 5.5, 89], [811, 7, 116], [1066, 9.1, 117], [842, 9.7, 87], [670, 7.2, 93], [790, 8.1, 97], [842, 8.2, 103], [490, 7.2, 68], [610, 15.6, 39.3],
               [324, 3.8, 84],  [319, 4.6, 70], [368, 3.6, 103], [323, 3.2, 101], [366, 3.5, 105], [385, 12.8, 39], [334, 12, 37], [365, 13.4, 35],
               [841, 8.76, 96], [675, 7.35, 92], [776, 8.08, 96], [704, 7.18, 98], [701, 7.01, 100], [1300, 23.64, 55], 
               [1068, 22.72, 47], [772, 19.79, 39], [1990, 12.28, 162], [675, 7.67, 88], [436, 5.74, 76]]
...
Ridge_model = r"models_scores_MA_SA_ECSA/OctaDataSet_Seed334_model_1_Ridge_MA_SA_ECSA.pkl"
XGB_model = r"models_scores_MA_SA_ECSA/OctaDataSet_Seed334_model_4_XGBRegressor_MA_SA_ECSA.pkl"
...
for i in models_list:
    ...
    importances_mean.to_excel(f"{DIR_Data}/{str(type(loaded_model).__name__)}_Feature_importance_MA_SA_ECSA.xlsx")
