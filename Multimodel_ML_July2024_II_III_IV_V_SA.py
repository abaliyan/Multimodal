...
def Training_Testing_models (listofmodels, X, Y, foldName):
    ...
    df_results.to_excel(f"{output_filename}{foldName}_SA_model_mse_scores.xlsx", index=False)
...
test_y_lst =  [4.9, 6, 17.1, 28, 9.1, 8.2, 15.6, 3.6, 13.4]
        ...
        if foldName == 'OctaDataSet':
            for model_data in model_data_list:
                ...
                for selected_data in permutations:
                    ...
                    with open(f'{DIR_Data}/{fname}_feature_importance_SA.txt', 'a') as file:
            ...
            results_df.to_excel(f"{DIR_Data}/Aug_Predicted_Octa_SA.xlsx", index=False)


# 📂 Load your model
model_filename = f"{DIR_Data}/models_scores_SA/SingleDataSet_model_1_LinearRegression_SA.pkl"
...
for slice_name, (start, end) in slices.items():
    ...
    # Save logs
    log_file = f'{DIR_Data}/Aug_Predicted_data_{slice_name}_{file_ext}_SA.txt'
    csv_file = f'{DIR_Data}/predicted_scores_{slice_name}_{file_ext}_SA.csv'
