...
def Training_Testing_models (listofmodels, X, Y, foldName):
    ...
    df_results.to_excel(f"{output_filename}{foldName}_ECSA_model_mse_scores.xlsx", index=False)
...
# # Prepare the Training Dataset [Features]   'y6', 'y7' is missiong --->
...
test_y_lst = [76, 56, 31, 8, 117, 103, 39.3, 103, 35, 96, 47, 162, 76]
...
if customflag == True:
    ...
    for foldName, model_data_list in models_dict.items():
        if foldName == 'OctaDataSet':
            ...
            results_df.to_excel(f"{DIR_Data}/Aug_Predicted_Octa_ECSA.xlsx", index=False)


# 📂 Load your model
model_filename = f"{DIR_Data}/models_scores_ECSA/SingleDataSet_model_1_LinearRegression_ECSA.pkl"
...
for slice_name, (start, end) in slices.items():
    ...
    # Save logs
    log_file = f'{DIR_Data}/Aug_Predicted_data_{slice_name}_{file_ext}_ECSA.txt'
    csv_file = f'{DIR_Data}/predicted_scores_{slice_name}_{file_ext}_ECSA.csv'
